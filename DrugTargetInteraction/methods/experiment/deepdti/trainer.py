...
class Trainer():
    def __init__(self, epoch=100, batch_size=256, ckpt_dir="./temp/",
            optimizer='adam',l2=1e-3, lr=1e-5, scheduler='cosineannealing',uniprot2pssm={},ikey2smiles={},molecule_dict={}):
    ...
    def train(self, model, pred_model, edges, train_evaluator, dev_edges, dev_evaluator):
        ...
        for epoch in range(1, self.train_epoch + 1):
            ...
            for batch_ in range(batch_per_epoch):
                ...
                #batch_chem_repr = [ikey2smiles[pair[0]] for pair in batch_train_pairs] #smiles
                batch_chem_repr = [pair[0] for pair in batch_train_pairs] #
                batch_chem_embed = get_embedding(self.model,'chemical',batch_chem_repr) #ikeys
            ...
            if epoch:
                ...
                if dev_pc > prev_dev_pc:
                    prev_dev_pc = dev_pc #new best pearson correlation
                    best_epoch = epoch
  ...
  BatchTrainTime {:.1f}, BatchOptimTime {:.1f}".format(epoch,
      chem_embed_time,prot_embed_time,batch_prep_time,batch_train_time,batch_optim_time))
        logging.info("Best Dev PC {} at epoch {}".format(prev_dev_pc,best_epoch))
