...
class PSSMTemporalConv(nn.Module):

    def __init__(self,dropout_p=0.3):
        super(PSSMTemporalConv, self).__init__()
        self.conv1 = nn.Conv1d(700,700, kernel_size=20, stride=1, padding=0)
    ...
    def forward(self, x):
        x = x.view(-1, 700, 20)
        x = F.relu(self.bn1(self.conv1(x)))#(N,700,1)
        x = x.view(-1,700)
        x = self.transform(x)
        x = x.view(-1,16,64)
        return x

class PSSMEncoding(nn.Module):

    def __init__(self):
        super(PSSMEncoding, self).__init__()
        self.conv1 = nn.Conv2d(1, 8, kernel_size=(1,20), stride=1, padding=0) #(-1,8,700,1)
        self.bn1 = nn.BatchNorm2d(8)
        self.conv1_down = nn.Conv1d(8, 8, kernel_size=4, stride=2, padding=2) #(-1,8,350)
        self.conv2 = nn.Conv1d(8, 16, kernel_size=4, stride=1, padding=2) #(-1,16,350)
        self.conv2_down = nn.Conv1d(16, 16, kernel_size=4, stride=2, padding=2) #(-1,16,175)
        self.bn2 = nn.BatchNorm1d(16)

        self.conv3 = nn.Conv1d(16, 32, kernel_size=4, stride=1, padding=2) #(-1,32,175)
        self.bn3 = nn.BatchNorm1d(32)
        self.maxpool3 = nn.MaxPool1d(5,stride=5,padding=2) #(-1,32,36)

    def forward(self, x):
        x = x.view(-1, 1, 700, 20)
        x = F.relu(self.bn1(self.conv1(x))) #(-1,8,700,1)
        x = torch.squeeze(x,dim=3) #(-1,8,700)
        x = self.conv1_down(x) #(-1,8,350)
        x = F.relu(self.bn2(self.conv2_down(self.conv2(x)))) #(-1,16,175)
        x = self.maxpool3(self.bn3(self.conv3(x))) #(-1,32,36)
        return x
...
class DeepDTI(nn.Module):
    def __init__(self, hidden_size, molecule_dict, dropout_chem=0.0, dropout_prot=0.0):
        super(DeepDTI, self).__init__()
        self.hidden_size = hidden_size
        self.pad = dict()
        from fingerprint.models_preprocess import NeuralFingerprint
        import fingerprint.features as fp_feature
        type_map = dict(batch='molecule', node='atom', edge='bond')
        conv_layer_sizes = [20, 20, 20, 20] #better than [32,64,64,32]
        output_size = hidden_size
        degrees = [0, 1, 2, 3, 4, 5]
        fp_model = NeuralFingerprint(
        #fp_model = RNNFingerprint(
        ...
        #self.add_module('protein', PSSMEncoding())
        #self.add_module('protein', PSSMEmbedding(hidden_size)) #better than PSSMEncoding
        self.add_module('protein', PSSMTemporalConv(dropout_p=dropout_prot)) 
...
class AttentivePooling(nn.Module):
    ...
    def __init__(self, hidden_size):
        ...
        self.hidden_size = hidden_size
        #self.param = nn.Parameter(torch.zeros(hidden_size, 175)) #AP params for chem-PSSM, 36 for pssmembedding
        self.param = nn.Parameter(torch.zeros(64, 64)) #AP params for chem-PSSM, 36 for pssmembedding
    ...
    def forward(self, first, second):
        ...
        #param = self.param.expand(first.size(0), self.hidden_size, 175) #36 for pssmencoding
        param = self.param.expand(first.size(0), 64, 64) 
...
class Predict(nn.Module):
    ...
    entity types.
    """
    def __init__(self, hidden_size, attn_dropout):
        super(Predict, self).__init__()
        self.hidden_size = hidden_size

        self.add_module('chemical', EmbeddingTransform(64, hidden_size, hidden_size, dropout_p=attn_dropout))
        #self.add_module('protein', EmbeddingTransform(175, hidden_size, hidden_size, dropout_p=dropout_p)) #36 for pssmencoding
        self.add_module('protein', EmbeddingTransform(64, hidden_size, hidden_size, dropout_p=attn_dropout)) #for temporal conv
