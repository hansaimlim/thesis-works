...
from trainer import TrainerForInference as Trainer
...
parser.add_argument('--datapath', default='data/activity/no_split',help='Path to the training dataset.')
...
parser.add_argument('--max_eval_steps', default=10000, type=int, help="Max evaluation steps. (nsamples=batch*steps)")
...
if __name__ == '__main__':
    ...
    if opt.prediction_mode == 'binary':
        opt.traindata = os.path.join(opt.datapath,'kinase_binary.tsv')
    else:
        opt.traindata = os.path.join(opt.datapath,'kinase_consolidated.tsv') 
    ...
    uniprot2triplets=load_json('data/protein/uniprot2triplets.json')
    ...
    record_dict = trainer.train(edges, train_evaluator, force_debug=opt.force_debug)
