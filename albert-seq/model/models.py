...
class MolecularGraphCoupler(nn.Module):
    ...
    def __init__(self,
        ...
        if protein_embedding_type.lower() in ['albert','bert','nlp']:
            self.proteinEmbedding = AlbertResnet(albertconfig=albertconfig,
                                tokenizer=tokenizer,
                                ckpt_path=ckpt_path,
                                albert_layers_frozen=albert_layers_frozen)
            self.protein_embedding_type = 'albert'
            
        elif protein_embedding_type.lower() in ['lstm','recurrent','rnn','ping']:
            ...
            self.linear_protein_pooler = EmbeddingTransform(384*lstm_hidden_size,128,lstm_hidden_size,dropout_p=ap_dropout)
            self.linear_ligand_pooler = EmbeddingTransform(384*output_size,128,output_size,dropout_p=ap_dropout)
            self.protein_embedding_type = 'lstm'
            prot_hidden_size = lstm_hidden_size
        else: #pssm representation
            self.proteinEmbedding = ResnetEncoderSuperTiny(1)
            self.protein_embedding_type = 'pssm'
            #prot_hidden_size = 256
    ...
    def forward(self, batch_input, **kwargs):
        ...
        #logging.debug("MolecularGraphCoupler: input ligand {}".format(batch_input['ligand'].size()))
        protein_vector = self.proteinEmbedding(batch_input['protein'])
        protein_repr, protein_vector = protein_vector[0], protein_vector[1] #comment it out
        ...
        #return protein_repr, pv_before_pool, protein_vector, logits #comment it out
        return logits
...
class AlbertResnet(nn.Module):
    def __init__(self, albertconfig=None, 
        ...
        if self.config is None:
            ...
            self.config = AlbertConfig.from_pretrained('data/albertdata/albertconfig/albert_config_tiny_google_singlet.json')
        self.tokenizer = tokenizer
        if self.tokenizer is None:
            #self.tokenizer = BertTokenizer.from_pretrained('data/albertdata/vocab/pfam_vocab_triplets.txt')
            self.tokenizer = BertTokenizer.from_pretrained('data/albertdata/vocab/pfam_vocab.txt')

        if ckpt_path is None:
            ckpt_path = '/raid/home/hansaimlim/Pfam/albert_google/pretrained_pfam_singlet_df10/model.ckpt-3300000'
    ...
    def forward(self, batch_input, **kwargs):
        ...
        return albert_outputs[0], logits
        #return logits
