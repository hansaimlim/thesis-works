...
def load_ikey2smiles():
    file_path='../../data/Integrated/chemicals/integrated_chemicals.tsv'
    ikey2smiles={}
...
def load_edges_from_file(edgefile,allowed_uniprots=None,sep=',',header=True):
    #default data format:
...
def load_uniprot2pssm(max_len=512,padding=0):
    ...
    for gpcr_pssm_file in os.listdir(os.path.join(base_path,'gpcr_pssm_uniref50')):
        ...
        with open(os.path.join(os.path.join(base_path,'gpcr_pssm_uniref50'),gpcr_pssm_file),'r'):
            for line in f:
                line=line.strip().lstrip().split()
                if len(line)==0: #empty line
                    continue
